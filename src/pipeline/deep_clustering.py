...
class DeepClusteringModel:
    """
    A class for deep separated and embedded clustering.
    ...
    def pretrain_autoencoder(self, input_frame, save_dir, optimizer='adam', epochs=10, verbose=0, batch_size=64):
        ...
        self.autoencoder.save(
            save_dir / utils.file_title(f'AE Model features{self.n_latent_features} loss{final_loss}', '.keras',
                                           short=True))
    ...
    def plot_log_evaluation(log_file_path, smoothing_window=50, log_save_step=10):
        ...
        for ind, (metric, direction) in enumerate(zip(metrics, optim_directions)):
            ...
            utils.print_optimal_index(smoothed, metric, direction)
    ...
    def fit(self, X_train, log_dir, save_dir, epochs=500, eval_epochs=10, save_epochs=100,
        ...
        logfile = open(log_dir / utils.file_title("DEC LOG", ".csv"), 'w')
            ...
            if epoch % save_epochs == 0:
                ...
                if visualize_dir is not None:
                    ...
                    self.isomap_plot(input_data=X_train, title=title,
                                     save_title=visualize_dir / utils.file_title(title, ".png"),
                                     hidden=True)

                print(f'Saving interim DEC model to {save_dir}...')
                self.model.save(
                    save_dir / utils.file_title(f'Epoch{epoch} DEC Model', '.keras', short=False))
        ...
        self.model.save(save_dir / utils.file_title(
        ...
        if visualize_dir is not None:
            ...
            today = utils.file_title("", short=True)[:8]
            file_titles = [visualize_dir / f for f in os.listdir(visualize_dir) if f[:8] == today and f[-4:] == ".png"]
            file_titles.sort()
            # save such as gif:
            gif_path = visualize_dir / utils.file_title("Animated Isomap Training Progress", ".gif", short=True)
...
class DataPreprocessor:
    """
    A class to remove missing values and outliers, analyse feature significance, standardize features
    and plot feature space overviews for optimal clustering input data.
    """
    def __init__(
            self,
            data: pd.DataFrame,
    ...
    def plot_loadings_heatmap_pca(self, threshold=0, show_annotations=False,
        ...
        if self.save_fig:
            if save_title is None:
                ...
                save_title = self.plot_path / utils.file_title("Loadings Heatmap")
            plt.savefig(save_title)
    ...
    def plot_isomap_overview(self, title='Isomap', save_title=None, **embedding_kwargs) -> None:
        ...
        if self.save_fig:
            if save_title is None:
                if self.plot_path is None:
                    raise Exception("Plot path needs to be defined!")
                save_title = self.plot_path / utils.file_title("Isomap Overview")
            plt.savefig(save_title)